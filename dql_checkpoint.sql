-- DQL Checkpoint: Data Query Language
-- Tables: Customer(Customer_id, customer_Name, customer_Tel)
...
SELECT COUNT(*) AS total_orders, AVG(total_amount) AS avg_amount, MAX(total_amount) AS max_amount, MIN(total_amount) AS min_amount FROM Orders;

-- 5. For each product_id, display the number of orders
SELECT Product_id, COUNT(*) AS number_of_orders FROM Orders GROUP BY Product_id;

-- 6. Display the customer_id which has more than 2 orders
SELECT Customer_id, COUNT(*) AS number_of_orders FROM Orders GROUP BY Customer_id HAVING COUNT(*) > 2;

-- 7. For each month of the 2020 year, display the number of orders
SELECT MONTH(OrderDate) AS month, COUNT(*) AS number_of_orders FROM Orders WHERE YEAR(OrderDate) = 2020 GROUP BY MONTH(OrderDate) ORDER BY month;

-- 8. For each order, display the product_name, the customer_name and the date of the order
SELECT p.product_name, c.customer_Name, o.OrderDate FROM Orders o JOIN Customer c ON o.Customer_id = c.Customer_id JOIN Product p ON o.Product_id = p.Product_id;
